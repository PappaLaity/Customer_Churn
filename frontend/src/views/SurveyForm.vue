<!-- <template>
    <div class="w-screen h-screen flex items-center justify-center bg-gray-100">
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <h2 class="text-xl font-bold mb-4 text-center">Customer Survey</h2>

            <input v-model="name" placeholder="Nom" class="w-full border px-3 py-2 rounded mb-3" />

            <input v-model="email" placeholder="Email" class="w-full border px-3 py-2 rounded mb-3" />
            <textarea v-model="message" placeholder="Message" class="w-full border px-3 py-2 rounded mb-3"></textarea>
            <button @click="send" class="w-full bg-green-600 text-white py-2 rounded hover:bg-green-700 mb-2">
                Envoyer
            </button>
        </div>
    </div>
...
export default {
  ...
  methods: {
    send () {
      ...
      alert('Données envoyées (mock) ✅')
    }
  }
}
</script> -->
<template>
  <div class="w-screen min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
    ...
    <div class="relative w-full max-w-2xl max-h-[90vh] flex flex-col">
      <div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl p-6 md:p-8 flex flex-col h-full">
        ...
        <transition name="slide">
          <div v-if="successMessage" class="mb-4 p-3 bg-green-500/20 border border-green-500/50 rounded-lg flex items-start gap-2 flex-shrink-0">
        ...
        <!-- Message d'erreur -->
        <transition name="slide">
          <div v-if="errorMessage" class="mb-4 p-3 bg-red-500/20 border border-red-500/50 rounded-lg flex items-start gap-2 flex-shrink-0">
        ...
        <form @submit.prevent="send" class="flex-1 overflow-y-auto space-y-4 pr-2">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="group">
              ...
              <input
                v-model.number="formData.tenure"
                type="number"
                placeholder="0"
                min="0"
                class="w-full bg-white/5 border border-white/20 focus:border-blue-500 focus:bg-white/10 focus:outline-none text-white placeholder-slate-400 rounded-lg px-3 py-2 text-sm transition-all duration-200"
            ...
            <div class="group">
              ...
              <select
                v-model="formData.internetService"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="group">
              ...
              <input
                v-model.number="formData.monthlyCharges"
                type="number"
                placeholder="0"
                min="0"
                step="0.01"
            ...
            <div class="group">
              ...
              <input
                v-model.number="formData.totalCharges"
                type="number"
                placeholder="0"
                min="0"
                step="0.01"
          ...
          <div class="space-y-2 pt-2">
            <label class="flex items-center gap-2 cursor-pointer group">
              <input
                v-model="formData.paperlessBilling"
                type="checkbox"
                class="w-4 h-4 rounded border-white/20 bg-white/5 text-blue-600 focus:ring-blue-500 cursor-pointer"
              />
              <span class="text-slate-300 text-sm group-hover:text-white transition-colors">Facturation sans papier</span>
            </label>

            <label class="flex items-center gap-2 cursor-pointer group">
              <input
                v-model="formData.noInternetService"
                type="checkbox"
                class="w-4 h-4 rounded border-white/20 bg-white/5 text-blue-600 focus:ring-blue-500 cursor-pointer"
              />
              <span class="text-slate-300 text-sm group-hover:text-white transition-colors">Aucun service internet</span>
            </label>
          </div>
          ...
          <div class="group">
            ...
            <textarea
              ...
              rows="3"
              class="w-full bg-white/5 border border-white/20 focus:border-blue-500 focus:bg-white/10 focus:outline-none text-white placeholder-slate-400 rounded-lg px-3 py-2 text-sm transition-all duration-200 resize-none"
            ></textarea>
          </div>
        </form>
