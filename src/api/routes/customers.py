...
"""
Routes pour récupérer les données clients (depuis PostgreSQL)
"""
...
async def get_customers_infos():
    """
    Récupère les données clients depuis la base PostgreSQL
    (table: predictions)

    Retourne les individus les plus récents d'abord.
    """
    ...
    try:
        ...
        with Session(engine) as session:
            query = text("""
                SELECT id, tenure, monthly_charges, prediction, created_at
                FROM predictions
