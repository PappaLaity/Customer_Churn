...
#     # Replace missing TotalCharges with MonthlyCharges
...
def preprocess_data():
    ...
    # Fill NaNs in TotalCharges using MonthlyCharges
    df["TotalCharges"] = df["TotalCharges"].fillna(df["MonthlyCharges"])
    ...
    object_cols = [
        c for c in df.select_dtypes(include=["object"]).columns if c != "Churn"
    ]
    encoders = {}
    ...
    internet_cols = [
        c for c in df.columns if "No internet service" in c or "InternetService_No" in c
    ]
    if internet_cols:
