...
def preprocess_data():
    ...
    """
    # Base directory for all pipeline outputs. Matches AIRFLOW_HOME so the
    # Airflow runtime keeps writing under /opt/airflow, while tests can point
    ...
    df.to_csv(os.path.join(preprocessed_dir, "preprocessed.csv"), index=False)
